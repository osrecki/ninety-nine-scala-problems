 ...
 * res0: List[List[Symbol]] = List(List('o), List('d, 'e), List('d, 'e), List('m, 'n),
 ...
 *    frequency; i.e. in the default, sorting is done in ascending order, lists with
 *    rare lengths are placed, others with a more frequent length come later.
 *
 * ==Example==
 * {{{
 ...
 */
object Problem28 {
  def lsort[A](lists: Seq[Seq[A]]): Seq[Seq[A]] = lists.sortBy(_.size)

  def lsortFreq[A](lists: Seq[Seq[A]]): Seq[Seq[A]] = {
    val sizesFrequencies = lists.map(_.size).groupBy(identity).mapValues(_.length)

    lists.sortBy { list => sizesFrequencies(list.size) }
  }
}
