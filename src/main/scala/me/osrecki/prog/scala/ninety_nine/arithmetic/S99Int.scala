...
class S99Int(val number: Int) {
  ...
  def primeFactors: Seq[Int] = {
    @tailrec
    def primeFactorsRec(n: Int, candidates: Seq[Int], factors: Seq[Int]): Seq[Int] = {
      if (n < 2) factors
      else if (n.isPrime) n +: factors
      else if (n % candidates.head == 0) primeFactorsRec(n / candidates.head, candidates, candidates.head +: factors)
      else primeFactorsRec(n, candidates.tail, factors)
    }

    primeFactorsRec(number, primes, Seq.empty[Int]).reverse
  }
